...
// 操作日志
// ============================================================
async function logOperation(env, operator, action, targetType, targetId, detail = null) {
  try {
    await env.NAV_DB.prepare(
      `INSERT INTO operation_logs (operator, action, target_type, target_id, detail) VALUES (?, ?, ?, ?, ?)`
    ).bind(operator || 'system', action, targetType, targetId, detail ? JSON.stringify(detail) : null).run();
...
async function checkSingleUrl(url, retries = 2) {
  ...
  try {
    ...
    if (!resp.ok && [405, 403, 404, 501, 503].includes(resp.status)) {
...
const api = {
    ...
    try {
      ...
      if (path === '/site/config') {
        ...
        if (method === 'PUT') {
          ...
          await logOperation(env, 'admin', 'update', 'config', 0, config);
        ...
        if (method === 'PUT') {
          ...
          if (body.rename) {
            ...
            try {
              ...
              await logOperation(env, 'admin', 'rename_category', 'category', 0, { from: categoryName, to: newName });
      ...
      if (path === '/health/check-batch' && method === 'POST') {
        ...
        await logOperation(env, 'admin', 'health_check_batch', 'site', 0, { count: ids.length });
      ...
      // 启动全量检查
      if (path === '/health/check-all' && method === 'POST') {
        if (!(await isAdminAuthenticated(request, env))) return this.errorResponse('Unauthorized', 401);
        return await this.startFullHealthCheck(request, env, ctx);
      }

      // 获取任务进度
      if (path.startsWith('/health/task/') && method === 'GET') {
        if (!(await isAdminAuthenticated(request, env))) return this.errorResponse('Unauthorized', 401);
        const taskId = path.replace('/health/task/', '');
        const data = await env.NAV_AUTH.get(`health_task_${taskId}`);
        if (!data) return this.errorResponse('任务不存在或已过期', 404);
        return new Response(JSON.stringify({ code: 200, data: JSON.parse(data) }), {
          headers: { 'Content-Type': 'application/json' }
        });
      }

      // ✅ 新增：获取任务已检查链接列表（用于逐条更新图标）
      if (path === '/health/task-links' && method === 'GET') {
        if (!(await isAdminAuthenticated(request, env))) return this.errorResponse('Unauthorized', 401);
        const taskId = url.searchParams.get('taskId');
        if (!taskId) return this.errorResponse('缺少 taskId', 400);
        const taskData = await env.NAV_AUTH.get(`health_task_${taskId}`);
        if (!taskData) return this.errorResponse('任务不存在', 404);
        const task = JSON.parse(taskData);
        const startTime = new Date(task.created_at).toISOString();
        const { results } = await env.NAV_DB.prepare(
          `SELECT id, health_status, health_error, health_checked_at FROM sites WHERE datetime(health_checked_at) > datetime(?)`
        ).bind(startTime).all();
        return new Response(JSON.stringify({ code: 200, data: results }), {
          headers: { 'Content-Type': 'application/json' }
        });
      ...
      // 暂停/恢复
      if (path.startsWith('/health/task/') && path.endsWith('/pause') && method === 'POST') {
        if (!(await isAdminAuthenticated(request, env))) return this.errorResponse('Unauthorized', 401);
        const taskId = path.replace('/health/task/', '').replace('/pause', '');
        const data = await env.NAV_AUTH.get(`health_task_${taskId}`);
        if (!data) return this.errorResponse('任务不存在', 404);
        const task = JSON.parse(data);
        task.status = 'paused';
        await env.NAV_AUTH.put(`health_task_${taskId}`, JSON.stringify(task));
        return new Response(JSON.stringify({ code: 200, message: '已暂停' }), {
          headers: { 'Content-Type': 'application/json' }
        });
      }

      if (path.startsWith('/health/task/') && path.endsWith('/resume') && method === 'POST') {
        if (!(await isAdminAuthenticated(request, env))) return this.errorResponse('Unauthorized', 401);
        const taskId = path.replace('/health/task/', '').replace('/resume', '');
        const data = await env.NAV_AUTH.get(`health_task_${taskId}`);
        if (!data) return this.errorResponse('任务不存在', 404);
        const task = JSON.parse(data);
        task.status = 'running';
        await env.NAV_AUTH.put(`health_task_${taskId}`, JSON.stringify(task));
        ctx.waitUntil(this.continueFullHealthCheck(env, taskId));
        return new Response(JSON.stringify({ code: 200, message: '已恢复' }), {
          headers: { 'Content-Type': 'application/json' }
        });
      }

      // 通知
      if (path === '/health/notification' && method === 'GET') {
        if (!(await isAdminAuthenticated(request, env))) return this.errorResponse('Unauthorized', 401);
        const data = await env.NAV_AUTH.get('health_notification');
        if (!data) return new Response(JSON.stringify({ code: 200, data: null }), {
          headers: { 'Content-Type': 'application/json' }
        });
        return new Response(JSON.stringify({ code: 200, data: JSON.parse(data) }), {
          headers: { 'Content-Type': 'application/json' }
      ...
      if (path === '/health/notification' && method === 'DELETE') {
        if (!(await isAdminAuthenticated(request, env))) return this.errorResponse('Unauthorized', 401);
        await env.NAV_AUTH.delete('health_notification');
        return new Response(JSON.stringify({ code: 200, message: '通知已清除' }), {
          headers: { 'Content-Type': 'application/json' }
        });
      }

      // 删除失效链接（批量）
      if (path === '/health/delete-failed' && method === 'POST') {
        if (!(await isAdminAuthenticated(request, env))) return this.errorResponse('Unauthorized', 401);
        const body = await request.json();
        const { ids } = body;
        if (!ids || !Array.isArray(ids) || ids.length === 0) {
          return this.errorResponse('请提供要删除的链接 ID 列表', 400);
        }
        const placeholders = ids.map(() => '?').join(',');
        await env.NAV_DB.prepare(`DELETE FROM sites WHERE id IN (${placeholders})`).bind(...ids).run();
        await logOperation(env, 'admin', 'delete_failed_health', 'site', 0, { count: ids.length });
        return new Response(JSON.stringify({ code: 200, message: `已删除 ${ids.length} 个失效链接` }), {
          headers: { 'Content-Type': 'application/json' }
        });
      }

      // ---- 点击统计 ----
      if (path === '/stats/click' && method === 'POST') {
        const body = await request.json();
        const { siteId } = body;
        if (!siteId) return this.errorResponse('siteId required', 400);
        try {
          await env.NAV_DB.prepare(
            `INSERT INTO click_stats (site_id, click_date, click_count) VALUES (?, CURRENT_DATE, 1)
             ON CONFLICT(site_id, click_date) DO UPDATE SET click_count = click_count + 1`
          ).bind(siteId).run();
          return new Response(JSON.stringify({ code: 200, message: 'ok' }), {
            headers: { 'Content-Type': 'application/json' }
          });
        } catch (e) {
          return this.errorResponse(`统计失败: ${e.message}`, 500);
        }
      }

      if (path === '/stats/top5' && method === 'GET') {
        if (!(await isAdminAuthenticated(request, env))) return this.errorResponse('Unauthorized', 401);
        try {
          const { results } = await env.NAV_DB.prepare(
            `SELECT s.id, s.name, s.url, s.catelog, COALESCE(SUM(cs.click_count), 0) as total_clicks
             FROM sites s
             LEFT JOIN click_stats cs ON s.id = cs.site_id
             GROUP BY s.id
          ...
          return this.errorResponse(`获取统计失败: ${e.message}`, 500);
        }
      }
  ...
  async getConfig(request, env, ctx, url) {
    ...
    try {
      let query = `SELECT id, name, url, logo, \`desc\`, catelog, sort_order, create_time, update_time,
         health_status, health_error, datetime(health_checked_at, '+8 hours') as health_checked_at
  ...
  async approvePendingConfig(request, env, ctx, id) {
    try {
      ...
      await logOperation(env, 'admin', 'approve', 'pending_site', id, { name: config.name });
  ...
  async rejectPendingConfig(request, env, ctx, id) {
    try {
      const { results } = await env.NAV_DB.prepare('SELECT * FROM pending_sites WHERE id = ?').bind(id).all();
      const name = results.length > 0 ? results[0].name : id;
      await env.NAV_DB.prepare('DELETE FROM pending_sites WHERE id = ?').bind(id).run();
      await logOperation(env, 'admin', 'reject', 'pending_site', id, { name });
  ...
  async createConfig(request, env, ctx) {
    try {
      ...
      await logOperation(env, 'admin', 'add', 'site', insert.meta?.last_row_id || 0, { name: sanitizedName });
  ...
  async updateConfig(request, env, ctx, id) {
    try {
      ...
      await logOperation(env, 'admin', 'edit', 'site', parseInt(id), { name: sanitizedName });
  ...
  async deleteConfig(request, env, ctx, id) {
    try {
      const { results } = await env.NAV_DB.prepare('SELECT name FROM sites WHERE id = ?').bind(id).all();
      const name = results.length > 0 ? results[0].name : id;
      const del = await env.NAV_DB.prepare('DELETE FROM sites WHERE id = ?').bind(id).run();
      await logOperation(env, 'admin', 'delete', 'site', parseInt(id), { name });
  ...
  async batchSortConfig(request, env, ctx) {
    try {
      ...
      await logOperation(env, 'admin', 'batch_sort', 'site', 0, { count: items.length });
  ...
  async batchDeleteSites(request, env, ctx) {
    try {
      ...
      const placeholders = ids.map(() => '?').join(',');
      await env.NAV_DB.prepare(`DELETE FROM sites WHERE id IN (${placeholders})`).bind(...ids).run();
      await logOperation(env, 'admin', 'batch_delete', 'site', 0, { count: ids.length });
  ...
  async batchUpdateCategory(request, env, ctx) {
    try {
      ...
      await logOperation(env, 'admin', 'batch_category', 'site', 0, { category: category.trim(), count: ids.length });
  ...
  async importConfig(request, env, ctx) {
    try {
      ...
      await logOperation(env, 'admin', 'import', 'site', 0, { imported, skipped });
  ...
  // ✅ 修改：增加 created_at 字段
  async startFullHealthCheck(request, env, ctx) {
    try {
      const { results } = await env.NAV_DB.prepare('SELECT id FROM sites').all();
      if (results.length === 0) return this.errorResponse('没有可检查的链接', 400);
      ...
      await env.NAV_AUTH.put(`health_task_${taskId}`, JSON.stringify(taskData), { expirationTtl: 86400 });
      ctx.waitUntil(this.continueFullHealthCheck(env, taskId));
      await logOperation(env, 'admin', 'health_check_all', 'site', 0, { total: allIds.length });
      return new Response(JSON.stringify({
        code: 200,
        taskId,
        total: allIds.length,
      ...
      return this.errorResponse(`启动全量检查失败: ${e.message}`, 500);
    }
  },

  async continueFullHealthCheck(env, taskId) {
    try {
      const data = await env.NAV_AUTH.get(`health_task_${taskId}`);
      if (!data) return;
      const task = JSON.parse(data);
      ...
      const BATCH_SIZE = 20;
      const ids = task.ids;
      let startIdx = task.currentIndex || 0;
      let checked = task.checked || 0;
      let failed = task.failed || 0;

      const startTime = Date.now();
      const MAX_RUNTIME_MS = 10 * 60 * 1000;

      while (startIdx < ids.length && task.status !== 'paused') {
        if (Date.now() - startTime > MAX_RUNTIME_MS) {
          console.warn('健康检查总超时，强制结束');
          break;
        }
        ...
        try {
          batchResults = await Promise.race([
            checkBatchSites(batchIds, env),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Batch timeout')), 30000))
          ]);
        } catch (err) {
          batchError = err;
          console.warn(`批处理失败 (ID: ${batchIds.join(',')}):`, err.message);
          batchResults = batchIds.map(id => ({ id, ok: false, status: 0, error: err.message }));
        }

        if (batchResults && Array.isArray(batchResults)) {
          const failedCount = batchResults.filter(r => !r.ok).length;
          failed += failedCount;
        ...
        task.failed = failed;
        task.currentIndex = startIdx;
        task.status = (startIdx >= ids.length) ? 'completed' : 'running';

        await env.NAV_AUTH.put(`health_task_${taskId}`, JSON.stringify(task));

        if (task.status === 'completed') {
          await env.NAV_AUTH.put('health_notification', JSON.stringify({
            unread: true,
            taskId,
          ...
          await logOperation(env, 'system', 'health_check_complete', 'site', 0, { total: ids.length, failed });
          break;
        }

        await new Promise(r => setTimeout(r, 500));

        const refreshed = await env.NAV_AUTH.get(`health_task_${taskId}`);
        if (refreshed) {
          const newTask = JSON.parse(refreshed);
          if (newTask.status === 'paused') break;
        }
      }

      if (task.status !== 'completed' && startIdx >= ids.length) {
      ...
      console.error('健康检查任务异常:', e);
      try {
        const task = JSON.parse(await env.NAV_AUTH.get(`health_task_${taskId}`));
        if (task) {
          task.status = 'failed';
          await env.NAV_AUTH.put(`health_task_${taskId}`, JSON.stringify(task));
        }
  ...
  <div class="app">
    ...
    <aside class="sidebar" id="sidebar">
      ...
      <nav class="sidebar-nav">
        ...
        <a href="#" class="nav-item" data-page="health" id="healthNav"><span class="nav-icon">❤️</span> 健康检查 <span id="healthBadge" class="badge" style="display:none;">●</span></a>
      ...
      <div class="page-container">
        ...
        <section id="page-dashboard" class="page active">
          ...
          <div class="category-section" style="margin-top:20px;">
            <div class="category-section-header">
              <span class="category-section-title">🔥 热门链接 TOP 5</span>
            </div>
            <div id="top5Container"><div style="padding:12px;color:#94a3b8;text-align:center;">加载中...</div></div>
          </div>
        </section>
        ...
        <section id="page-health" class="page">
          <div class="health-toolbar">
            <div class="health-stats">
              <span>✅ 健康: <strong id="healthOkStat">0</strong></span>
              <span>❌ 异常: <strong id="healthFailStat">0</strong></span>
              <span>⏳ 未检查: <strong id="healthUnknownStat">0</strong></span>
            </div>
            <div class="health-actions">
              <button id="fullHealthCheckBtn" class="btn-primary">🩺 全量检查</button>
              <button id="pauseHealthCheckBtn" class="btn-secondary" style="display:none;">⏸️ 暂停</button>
              <button id="resumeHealthCheckBtn" class="btn-secondary" style="display:none;">▶️ 恢复</button>
              <button id="deleteAllFailedBtn" class="btn-secondary" style="color:#ef4444;">🗑️ 删除全部失效</button>
            </div>
          </div>
          <div id="healthProgress" style="display:none;background:#fff;border-radius:10px;padding:16px;margin-bottom:16px;">
            <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
              <span id="healthProgressText">检查中...</span>
              <span id="healthProgressPercent">0%</span>
            </div>
            <div style="background:#e2e8f0;border-radius:8px;height:20px;overflow:hidden;position:relative;">
              <div id="healthProgressBar" style="height:100%;background:linear-gradient(90deg,#4f46e5,#818cf8);border-radius:8px;width:0%;transition:width 0.3s;"></div>
              <span id="healthProgressClose" style="position:absolute;right:4px;top:50%;transform:translateY(-50%);color:#fff;cursor:pointer;font-weight:bold;font-size:14px;background:rgba(0,0,0,0.3);padding:0 8px;border-radius:4px;">✕</span>
            </div>
          </div>
          <div class="table-wrapper">
            <table>
              <thead><tr>
                <th style="width:30px;"><input type="checkbox" id="healthSelectAll" /></th>
                <th>ID</th><th>名称</th><th>URL</th><th>健康值</th><th>检查时间</th><th>操作</th>
              </tr></thead>
              <tbody id="healthTableBody"></tbody>
            </table>
          </div>
          <div class="health-summary">💡 点击「全量检查」后，每个链接会逐个显示检查状态</div>
        </section>
...
.health-stats { display: flex; gap: 20px; font-size: 14px; }
.health-stats strong { font-size: 18px; }
.health-actions { display: flex; gap: 8px; }
.health-summary { margin-top: 12px; padding: 12px 16px; background: #fff; border-radius: 10px; font-size: 14px; color: #64748b; }
#healthProgress { background: #fff; border-radius: 10px; padding: 16px; margin-bottom: 16px; }
...
#top5Container .top5-item:last-child { border-bottom: none; }
#top5Container .rank { 
  font-weight: 700; 
  width: 28px; 
  color: #94a3b8;
...
#top5Container .rank-1 { color: #f59e0b; }
#top5Container .rank-2 { color: #94a3b8; }
#top5Container .rank-3 { color: #d97706; }
#top5Container .top5-name { flex: 1; font-weight: 500; }
#top5Container .top5-clicks { color: #64748b; font-size: 13px; }
#top5Container .top5-category { background: #f1f5f9; padding: 0 10px; border-radius: 10px; font-size: 12px; color: #64748b; }
...
@media (max-width: 768px) {
  ...
  .health-stats { justify-content: center; }
  .health-actions { justify-content: center; flex-wrap: wrap; }
...
// 健康检查
const healthTableBody = $('healthTableBody');
const healthOkStat = $('healthOkStat');
const healthFailStat = $('healthFailStat');
const healthUnknownStat = $('healthUnknownStat');
const fullHealthCheckBtn = $('fullHealthCheckBtn');
const pauseHealthCheckBtn = $('pauseHealthCheckBtn');
const resumeHealthCheckBtn = $('resumeHealthCheckBtn');
const deleteAllFailedBtn = $('deleteAllFailedBtn');
const healthProgress = $('healthProgress');
const healthProgressBar = $('healthProgressBar');
const healthProgressText = $('healthProgressText');
const healthProgressPercent = $('healthProgressPercent');
const healthProgressClose = $('healthProgressClose');
const healthSelectAll = $('healthSelectAll');

const top5Container = $('top5Container');
...
navItems.forEach(item => {
  item.addEventListener('click', function(e) {
    ...
    const titles = {
      ...
      categories:'分类管理', health:'健康检查', settings:'站点设置'
    };
    pageTitle.textContent = titles[page] || page;
    if (page === 'dashboard') { loadDashboard(); loadTop5(); }
    if (page === 'links') fetchConfigs();
    if (page === 'pending') fetchPending();
    if (page === 'categories') fetchCategories();
    if (page === 'health') { loadHealthPage(); checkHealthNotification(); }
...
async function saveCategoryOrder() {
  ...
  try {
    for (const item of items) {
      await fetch('/api/categories/' + encodeURIComponent(item.id), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sort_order: item.sort_order })
      });
    }
    showLoaderDone('分类排序已更新');
...
async function loadTop5() {
  try {
    const res = await fetch('/api/stats/top5');
    const data = await res.json();
    if (data.code === 200 && data.data) {
      const list = data.data;
      if (list.length === 0) {
        top5Container.innerHTML = '<div style="padding:12px;color:#94a3b8;text-align:center;">暂无点击数据</div>';
        return;
      }
      const rankEmojis = ['🥇', '🥈', '🥉', '4️⃣', '5️⃣'];
      top5Container.innerHTML = list.map((item, idx) => {
        const rankClass = idx === 0 ? 'rank-1' : idx === 1 ? 'rank-2' : idx === 2 ? 'rank-3' : '';
        return \`
          <div class="top5-item">
            <span class="rank \${rankClass}">\${rankEmojis[idx] || (idx+1)}</span>
            <span class="top5-name">\${escapeHTML(item.name)}</span>
            <span class="top5-category">\${escapeHTML(item.catelog || '未分类')}</span>
            <span class="top5-clicks">👆 \${item.total_clicks || 0} 次</span>
          </div>
        \`;
      }).join('');
    }
...
function renderConfigs(configs) {
  ...
  configs.forEach((config, idx) => {
    ...
    const checkedAt = formatDate(config.health_checked_at);
...
function handleDragEnd(e) {
  ...
  if (isFullView) {
    ...
    fetch('/api/config/batch-sort', {
      ...
      if (data.code === 200) {
        showLoaderDone('排序已更新');
        setTimeout(() => { hideLoader(); fetchConfigs(currentPage); }, 1200);
      } else {
        showLoaderDone(data.message);
        setTimeout(() => { hideLoader(); fetchConfigs(currentPage); }, 1500);
      }
    }).catch(() => {
      showLoaderDone('网络错误');
      setTimeout(() => { hideLoader(); fetchConfigs(currentPage); }, 1500);
  ...
  dragSrcRow = null; dragTargetRow = null;
}
...
async function loadHealthPage() {
  try {
    if (healthTaskId) {
      const res = await fetch('/api/health/task/' + healthTaskId);
      const data = await res.json();
      if (data.code === 200 && (data.data.status === 'running' || data.data.status === 'paused')) {
        healthTaskStartTime = data.data.created_at;
        startHealthPolling();
        return;
      }
    }
    const res = await fetch('/api/config?page=1&pageSize=10000');
    const data = await res.json();
    if (data.code === 200) {
      const sites = data.data || [];
      const ok = sites.filter(s => s.health_status === 1).length;
      const fail = sites.filter(s => s.health_status === 2).length;
      const unknown = sites.filter(s => s.health_status === 0 || s.health_status == null).length;
      healthOkStat.textContent = ok;
      healthFailStat.textContent = fail;
      healthUnknownStat.textContent = unknown;
      const checked = sites.filter(s => s.health_status > 0);
      renderHealthTable(checked);
    }
  } catch(e) { console.error(e); }
}

function renderHealthTable(sites) {
  healthTableBody.innerHTML = '';
  if (!sites || sites.length === 0) {
    healthTableBody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:#94a3b8;padding:30px;">暂无已检查的链接，请点击「全量检查」或对具体链接进行检查</td></tr>';
    return;
  }
  sites.forEach(s => {
    const row = document.createElement('tr');
    let healthHtml = '';
    if (s.health_status === 1) {
      healthHtml = '<span class="health-ok">🟢 正常</span>';
    } else if (s.health_status === 2) {
      const errMsg = s.health_error || '未知错误';
      healthHtml = '<span class="health-fail" title="异常原因: ' + escapeHTML(errMsg) + '">🔴 异常</span>';
    } else {
      healthHtml = '<span class="health-loading">⏳</span>';
    }
    row.innerHTML = \`
      <td><input type="checkbox" class="health-select" value="\${s.id}" /></td>
      <td>\${s.id}</td>
      <td>\${escapeHTML(s.name)}</td>
      <td><a href="\${escapeHTML(normalizeUrl(s.url)||'#')}" target="_blank">\${escapeHTML(s.url)}</a></td>
      <td>\${healthHtml}</td>
      <td>\${formatDate(s.health_checked_at)}</td>
      <td class="actions">
        <button class="del-btn health-del" data-id="\${s.id}">删除</button>
      </td>
    \`;
    healthTableBody.appendChild(row);
  });
  $$('.health-del').forEach(btn => btn.addEventListener('click', function() {
    const id = this.dataset.id;
    showConfirm('删除链接', '确定要删除该链接吗？', async function() {
      showLoader('删除中...');
      try {
        await fetch('/api/config/' + id, { method: 'DELETE' });
        showLoaderDone('删除成功');
        setTimeout(() => { hideLoader(); loadHealthPage(); loadDashboard(); }, 1200);
      } catch(e) {
        showLoaderDone('删除失败');
        setTimeout(hideLoader, 1500);
      }
  ...
  healthSelectAll.addEventListener('change', function() {
    $$('.health-select').forEach(cb => cb.checked = this.checked);
  });
}

// 全量健康检查
fullHealthCheckBtn.addEventListener('click', function() {
  showConfirm('全量健康检查', '将对所有链接进行健康检查，可能耗时较长，确认继续？', async function() {
    showLoader('正在启动全量检查...');
    try {
      const res = await fetch('/api/health/check-all', { method: 'POST' });
      const data = await res.json();
      if (data.code === 200) {
        healthTaskId = data.taskId;
        healthTaskStartTime = data.created_at;
        showLoaderDone('检查已启动');
        setTimeout(() => { hideLoader(); startHealthPolling(); }, 1200);
      } else {
        showLoaderDone(data.message);
        setTimeout(hideLoader, 1500);
      }
...
function startHealthPolling() {
  ...
  pauseHealthCheckBtn.style.display = 'inline-block';
  resumeHealthCheckBtn.style.display = 'none';
  fullHealthCheckBtn.disabled = true;

  if (healthPollInterval) clearInterval(healthPollInterval);
  healthPollInterval = setInterval(async function() {
    if (!healthTaskId) return;
    try {
      const taskRes = await fetch('/api/health/task/' + healthTaskId);
      const taskData = await taskRes.json();
      if (taskData.code !== 200) return;
      const task = taskData.data;

      const pct = task.total > 0 ? Math.round((task.checked / task.total) * 100) : 0;
      healthProgressBar.style.width = pct + '%';
      healthProgressText.textContent = \`检查中: \${task.checked}/\${task.total} 个\`;
      healthProgressPercent.textContent = pct + '%';

      // 获取所有站点，用于展示状态
      const sitesRes = await fetch('/api/config?page=1&pageSize=10000');
      const sitesData = await sitesRes.json();
      if (sitesData.code === 200) {
        const allSites = sitesData.data || [];
        const ok = allSites.filter(s => s.health_status === 1).length;
        const fail = allSites.filter(s => s.health_status === 2).length;
        const unknown = allSites.filter(s => s.health_status === 0 || s.health_status == null).length;
        healthOkStat.textContent = ok;
        healthFailStat.textContent = fail;
        healthUnknownStat.textContent = unknown;

        // 根据任务开始时间筛选本次任务已检查的链接
        const startTime = healthTaskStartTime || task.created_at;
        const checkedSites = allSites.filter(s => {
          if (!s.health_checked_at) return false;
          return new Date(s.health_checked_at).getTime() >= startTime;
        });
        const uncheckedSites = allSites.filter(s => {
          if (!s.health_checked_at) return true;
          return new Date(s.health_checked_at).getTime() < startTime;
        });
        const combined = [...checkedSites, ...uncheckedSites];
        renderHealthTable(combined);
      }

      if (task.status === 'completed') {
        clearInterval(healthPollInterval);
        healthPollInterval = null;
        healthProgressText.textContent = \`✅ 检查完成！异常 \${task.failed || 0} 个\`;
        healthProgressBar.style.width = '100%';
        healthProgressBar.style.background = 'linear-gradient(90deg, #22c55e, #16a34a)';
        pauseHealthCheckBtn.style.display = 'none';
        resumeHealthCheckBtn.style.display = 'none';
        fullHealthCheckBtn.disabled = false;
        healthBadge.style.display = 'block';
        loadHealthPage();
      } else if (task.status === 'paused') {
        healthProgressText.textContent = '⏸️ 已暂停';
        pauseHealthCheckBtn.style.display = 'none';
        resumeHealthCheckBtn.style.display = 'inline-block';
      }
    } catch(e) {
...
pauseHealthCheckBtn.addEventListener('click', async function() {
  if (!healthTaskId) return;
  try {
    await fetch('/api/health/task/' + healthTaskId + '/pause', { method: 'POST' });
    pauseHealthCheckBtn.style.display = 'none';
    resumeHealthCheckBtn.style.display = 'inline-block';
    healthProgressText.textContent = '⏸️ 已暂停';
  } catch(e) { console.error(e); }
...
resumeHealthCheckBtn.addEventListener('click', async function() {
  if (!healthTaskId) return;
  try {
    await fetch('/api/health/task/' + healthTaskId + '/resume', { method: 'POST' });
    pauseHealthCheckBtn.style.display = 'inline-block';
    resumeHealthCheckBtn.style.display = 'none';
    healthProgressText.textContent = '▶️ 恢复中...';
    startHealthPolling();
  } catch(e) { console.error(e); }
});

healthProgressClose.addEventListener('click', function() {
  healthProgress.style.display = 'none';
});

// 删除全部失效（健康页面选中）
deleteAllFailedBtn.addEventListener('click', function() {
  const checked = $$('.health-select:checked');
  const ids = Array.from(checked).map(cb => parseInt(cb.value));
  if (ids.length === 0) { showMessage('请选择要删除的失效链接', 'error'); return; }
  showConfirm(\`删除失效链接\`, \`确定要删除选中的 \${ids.length} 个失效链接吗？\`, async function() {
    showLoader(\`正在删除 \${ids.length} 个失效链接...\`);
    try {
      const res = await fetch('/api/health/delete-failed', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids })
      });
      const data = await res.json();
      if (data.code === 200) {
        showLoaderDone(\`已删除 \${ids.length} 个失效链接\`);
        setTimeout(() => { hideLoader(); loadHealthPage(); loadDashboard(); fetchConfigs(); }, 1200);
      } else {
        showLoaderDone(data.message);
        setTimeout(hideLoader, 1500);
      }
...
// 健康通知（红点）
// ============================================================
async function checkHealthNotification() {
  try {
    const res = await fetch('/api/health/notification');
    const data = await res.json();
    if (data.code === 200 && data.data && data.data.unread) {
      healthBadge.style.display = 'block';
      if (pages.health.classList.contains('active')) {
        await fetch('/api/health/notification', { method: 'DELETE' });
...
healthNav.addEventListener('click', function() {
  fetch('/api/health/notification', { method: 'DELETE' });
  healthBadge.style.display = 'none';
});
...
document.addEventListener('click', function(e) {
  const link = e.target.closest('.site-card a[href]');
  if (link && link.href && !link.href.startsWith('#') && link.href.startsWith('http')) {
    ...
    if (card) {
      ...
      if (id) {
        fetch('/api/stats/click', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ siteId: parseInt(id) })
        }).catch(() => {});
      }
    }
  }
...
checkHealthNotification();

setInterval(checkHealthNotification, 30000);
